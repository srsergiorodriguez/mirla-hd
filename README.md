...
#### A. `Protocol.csv`
Este archivo funciona como protocolo que define los atributos principales de los ítems de tu colección. Debe contener exactamente dos encabezados: `Attribute` y `Type` (escritos exactamente así). Cada fila define un campo de metadatos y le indica a Mirla cómo mostrarlo en la tabla de metadatos del elemento.
...
* **`ref`**: Referencias o enlaces cruzados a otros elementos dentro de la colección principal (pids separados por comas).
* **`ref:[NombreTabla]`**: Referencias a una tabla relacional secundaria. Por ejemplo, `ref:Tecnica` conectará automáticamente este atributo con los elementos definidos en `Metadata_Tecnica.csv`.
* **`latlong`**: Coordenadas geográficas formateadas como `"latitud, longitud"` (ej. `"4.7110, -74.0721"`). Se usa para mapas.

#### B. `Metadata.csv`
Esta es la hoja de cálculo maestra que contiene los datos principales de tu colección, una fila para cada ítem de la colección.
...
Si tu colección es más compleja y necesitas conectar diferentes tipos de elementos sin repetir información (por ejemplo, múltiples publicaciones que comparten las mismas técnicas o autores), Mirla te permite usar tablas secundarias.
...
* Los elementos en estas tablas secundarias deben seguir las mismas reglas básicas (tener un `pid` y un `label`).
...
El plugin Mirla Collection Generator lee tus archivos CSV (principales y secundarios), construye una api consumible por los elementos interactivos y genera automáticamente páginas individuales con visores de imágenes y tablas de metadatos para cada elemento de la colección.
...
3. Configura las dos opciones principales:
   ...
   * **Excluded Metadata:** Enumera los nombres de los atributos de tu CSV que *no* deseas que sean visibles para el público en la página de cada elemento (ej. `pid`, o notas internas como `_resena`).
...
El componente principal para explorar la colección. Muestra una cuadrícula de elementos con barra de búsqueda y filtros desplegables.
```html
<mirla-index type="" filters="medio_impresion, ciudad"></mirla-index>
```
* `type`: Si lo omites o lo dejas vacío, mostrará los elementos de tu `Metadata.csv` principal. Si escribes el nombre de una tabla secundaria (ej. `type="Tecnica"`), creará una galería exclusiva para esos elementos.
